...
class TestStageControlWidgetInitialization(unittest.TestCase):
    ...
    def test_on_initialize_stage_real_hw_fallback_to_mock(self):
        ...
        self.assertTrue(
            "Status: Error - Real HW init failed. Using mock stage for 'RealStage'."
            in self.widget.status_label.setText.call_args[0][0]
        )
    ...
    def test_on_disconnect_stage(self):
        ...
        self.widget.mm_config_file_input.setEnabled.assert_called_with(True)
        self.widget.stage_device_label_input.setEnabled.assert_called_with(
            True
        )  # Always true in _on_mock_hw_changed current form
