...
class MoveStageButton(QPushButton):
    def __init__(self, glyph: str, xmag: int, ymag: int, parent: QWidget | None = None):
        ...
        # Prepare stylesheet-safe paths *before* the f-string
        # to avoid SyntaxError on macOS.
        path_default = str(svg_path(glyph, color="rgb(0, 180, 0)")).replace("\\", "/")
        path_hover = str(svg_path(glyph, color="lime")).replace("\\", "/")
        path_pressed = str(svg_path(glyph, color="green")).replace("\\", "/")
        ...
        self.setStyleSheet(
            ...
            MoveStageButton {{
                ...
                image: url({path_default});
                font-size: 36px;
            }}
            MoveStageButton:hover:!pressed {{
                image: url({path_hover});
            }}
            MoveStageButton:pressed {{
                image: url({path_pressed});
            }}
            """
        )
