...
class AcquisitionGUI(QMainWindow):
    ...
    @Slot()
    def _on_snap(self):
        """Acquires and displays a single image."""
        if self.acquisition_in_progress:
            return

        mmc.setExposure(self.settings.camera_exposure_ms)

        # Using a separate thread to avoid freezing the GUI during snap
        snap_thread = threading.Thread(target=self._snap_and_display)
        snap_thread.start()

    def _snap_and_display(self):
        """Helper function to run snap in a thread."""
        try:
            mmc.snapImage()
            # We need to emit a signal to update the GUI from the main thread
            # For a simple snap, we can use a QTimer to do this.
            QTimer.singleShot(0, lambda: self.display_image(mmc.getImage(), MDAEvent()))
    ...
    def start_time_series(self):
        ...
        # REFACTORED: Pass the hardware interface to the worker
    ...
    def _finish_time_series(self):
        ...
        # REFACTORED: Use the hw_interface object for cleanup
    ...
    class AcquisitionWorker(QObject):
        """Worker object that runs the acquisition sequence in a separate thread."""

        signals = WorkerSignals()

        def __init__(self, gui_instance: "AcquisitionGUI", hw_interface: HardwareInterface):
            super().__init__()
            self.gui = gui_instance
            # REFACTORED: Store the hardware interface
            self.hw_interface = hw_interface
        ...
        def run(self):
            ...
            try:
                ...
                for t_point in range(params["num_time_points"]):
                    ...
                    # REFACTORED: Use hw_interface methods
                    (
                        galvo_amp,
                        galvo_center,
                        num_slices,
                    ) = self.hw_interface.calculate_galvo_parameters(self.gui.settings)

                    self.hw_interface.configure_devices_for_slice_scan(
                        self.gui.settings, galvo_amp, galvo_center, num_slices
                    )
                    ...
                    # REFACTORED: Use hw_interface method
                    self.hw_interface.trigger_slice_scan_acquisition()
                    ...
                    if not is_last_timepoint:
                        # REFACTORED: Use hw_interface method
                        self.hw_interface._reset_for_next_volume()
        ...
        def _process_images(self, t_point: int, num_slices: int, sequence, writer):
            """Pops images from the core and sends them for display/saving."""
            popped_images = 0
        ...
        def _create_mda_sequence(self, num_slices: int) -> MDASequence:
            """Creates a useq.MDASequence object for metadata."""
            step_size = self.gui.settings.step_size_um
            z_range = (num_slices - 1) * step_size if num_slices > 1 else 0.0
            return MDASequence(z_plan={"range": z_range, "step": step_size})  # type: ignore

        def _setup_writer(self, t_point: int, params: dict):
            """Initializes the OMETiffWriter if saving is enabled."""
        ...
        def _wait_for_interval(self, params: dict, start_time: float):
            """Waits for the specified time interval between volumes."""
