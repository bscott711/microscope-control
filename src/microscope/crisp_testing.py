...
class HardwareInterface:
    ...
    def set_p_objective_position(self, position_um: float, wait: bool = True): 
        ...
        try:
            ...
            print(f"Moved P Objective Piezo to {position_um:.3f} µm") # Added formatting
        except Exception as e:
    ...
    def _set_galvo_property(self, property_name: str, value: float):
        ...
        try:
            ...
            print(f"Set Galvo property '{property_name}' to {value:.4f}") # Added formatting
        except Exception as e:
    ...
    def move_crisp_o1_target_focus(self, z_um: float, wait: bool = True):
        ...
        try:
            ...
            print(f"Moved CRISP O1 target focus (stage {self.crisp_o1_focus_stage}) to {z_um:.2f} µm")
        except Exception as e:
    ...
    def set_crisp_state(self, crisp_autofocus_device_label: str, state: str) -> bool:
        ...
        try:
            ...
            allowed_states = mmc.getAllowedPropertyValues(crisp_autofocus_device_label, "CRISP State")
            if state not in allowed_states:
                print(f"Error: '{state}' is not an allowed state for {crisp_autofocus_device_label}. Allowed: {allowed_states}")
                return False
            ...
            if state.lower() in ["lock", "focus", "find rng", "logcal", "gaincal", "dcal", "ready"]: # Added "ready"
...
if __name__ == "__main__":
    ...
    try:
        ...
        crisp1_initial_state = hw_interface.get_crisp_state(hw_interface.crisp_o1_autofocus_device)
        print(f"CRISP O1 Initial State ({hw_interface.crisp_o1_autofocus_device}): {crisp1_initial_state if crisp1_initial_state is not None else 'N/A'}")
        
        crisp3_initial_state = hw_interface.get_crisp_state(hw_interface.crisp_o3_autofocus_device)
        print(f"CRISP O3 Initial State ({hw_interface.crisp_o3_autofocus_device}): {crisp3_initial_state if crisp3_initial_state is not None else 'N/A'}")
        ...
        if crisp1_initial_state == "Idle":
            print(f"Attempting to set {hw_interface.crisp_o1_autofocus_device} to '{target_crisp_state}'...")
            if hw_interface.set_crisp_state(hw_interface.crisp_o1_autofocus_device, target_crisp_state):
                crisp1_state_changed_to_ready = True # Mark that we changed it
                time.sleep(1) # Allow time for state change
                current_state = hw_interface.get_crisp_state(hw_interface.crisp_o1_autofocus_device)
                print(f"State after setting to '{target_crisp_state}': {current_state}")

                if current_state == target_crisp_state:
                    print(f"Attempting to set {hw_interface.crisp_o1_autofocus_device} back to 'Idle'...")
                    if hw_interface.set_crisp_state(hw_interface.crisp_o1_autofocus_device, "Idle"):
                        crisp1_state_changed_to_ready = False # Successfully returned to Idle
                        time.sleep(0.5)
                        print(f"State after setting back to 'Idle': {hw_interface.get_crisp_state(hw_interface.crisp_o1_autofocus_device)}")
                    else:
                        print(f"Failed to set {hw_interface.crisp_o1_autofocus_device} back to 'Idle'.")
                elif current_state is not None:
                     print(f"Warning: {hw_interface.crisp_o1_autofocus_device} state is '{current_state}' after attempting to set to '{target_crisp_state}'.")
            else:
                print(f"Failed to set {hw_interface.crisp_o1_autofocus_device} to '{target_crisp_state}'.")
        elif crisp1_initial_state is None:
            print(f"Could not get initial state for {hw_interface.crisp_o1_autofocus_device}, skipping its state change test.")
        else:
            print(f"{hw_interface.crisp_o1_autofocus_device} initial state is '{crisp1_initial_state}', not 'Idle'. Skipping '{target_crisp_state}' -> 'Idle' test sequence.")

        # --- Test CRISP O3 ---
        print(f"\n--- Testing CRISP State Changes ({hw_interface.crisp_o3_autofocus_device}) ---")
        if crisp3_initial_state == "Idle":
            print(f"Attempting to set {hw_interface.crisp_o3_autofocus_device} to '{target_crisp_state}'...")
            if hw_interface.set_crisp_state(hw_interface.crisp_o3_autofocus_device, target_crisp_state):
                crisp3_state_changed_to_ready = True # Mark that we changed it
                time.sleep(1) 
                current_state = hw_interface.get_crisp_state(hw_interface.crisp_o3_autofocus_device)
                print(f"State after setting to '{target_crisp_state}': {current_state}")

                if current_state == target_crisp_state:
                    print(f"Attempting to set {hw_interface.crisp_o3_autofocus_device} back to 'Idle'...")
                    if hw_interface.set_crisp_state(hw_interface.crisp_o3_autofocus_device, "Idle"):
                        crisp3_state_changed_to_ready = False # Successfully returned to Idle
                        time.sleep(0.5)
                        print(f"State after setting back to 'Idle': {hw_interface.get_crisp_state(hw_interface.crisp_o3_autofocus_device)}")
                    else:
                        print(f"Failed to set {hw_interface.crisp_o3_autofocus_device} back to 'Idle'.")
                elif current_state is not None:
                    print(f"Warning: {hw_interface.crisp_o3_autofocus_device} state is '{current_state}' after attempting to set to '{target_crisp_state}'.")
            else:
                print(f"Failed to set {hw_interface.crisp_o3_autofocus_device} to '{target_crisp_state}'.")
        elif crisp3_initial_state is None:
            print(f"Could not get initial state for {hw_interface.crisp_o3_autofocus_device}, skipping its state change test.")
        else:
            print(f"{hw_interface.crisp_o3_autofocus_device} initial state is '{crisp3_initial_state}', not 'Idle'. Skipping '{target_crisp_state}' -> 'Idle' test sequence.")
        ...
        if hw_interface:
            ...
            if crisp1_state_changed_to_ready: # If it was successfully changed to Ready and not back to Idle
                print(f"\nEnsuring {hw_interface.crisp_o1_autofocus_device} is returned to Idle state in finally block...")
                hw_interface.set_crisp_state(hw_interface.crisp_o1_autofocus_device, "Idle")
                print(f"Final {hw_interface.crisp_o1_autofocus_device} State: {hw_interface.get_crisp_state(hw_interface.crisp_o1_autofocus_device)}")
            
            if crisp3_state_changed_to_ready: # If it was successfully changed to Ready and not back to Idle
                print(f"\nEnsuring {hw_interface.crisp_o3_autofocus_device} is returned to Idle state in finally block...")
                hw_interface.set_crisp_state(hw_interface.crisp_o3_autofocus_device, "Idle")
                print(f"Final {hw_interface.crisp_o3_autofocus_device} State: {hw_interface.get_crisp_state(hw_interface.crisp_o3_autofocus_device)}")
